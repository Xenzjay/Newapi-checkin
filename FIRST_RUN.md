...
Session 过期时，在账号行点击“登录”打开站点登录页，完成登录后通过油猴/浏览器扩展重新导入配置；也可以直接点击“更新凭据”，粘贴新的 Session。Worker 会覆盖该账号的加密运行配置，并保留账号本身和历史运行记录。由于 HttpOnly Cookie 不能被普通网页脚本读取，登录页不会自动把 Cookie 回传到 Worker。

账号表单中的“分组”用于归类同一站点的多个账号或不同用途；“标签”支持多个值，用逗号分隔。保存后可以在账号健康状态面板按分组或标签筛选。
...
以上五项出现后，Worker Cron 会每分钟检查控制台中的自动签到计划，并在设定时间触发一次 GitHub Actions。GitHub Actions 页面保留 `workflow_dispatch`，仍可手动运行。
